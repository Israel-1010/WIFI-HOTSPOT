...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    console.error("[v0] Erro no OAuth Facebook:", error)
  ...
  try {
    ...
    console.log("[v0] Dados do usuário Facebook:", userData)
    ...
    if (existingUser) {
      ...
      const { data, error: updateError } = await supabase
        ...
        .update({
          ...
          foto_perfil: userData.picture?.data?.url,
    ...
    try {
      ...
      if (mikrotikClient) {
        console.log(
          "[v0] Iniciando login no Mikrotik para usuário:",
          usuarioSocial.email || usuarioSocial.nome_completo,
        )

        const username =
          (usuarioSocial.email || usuarioSocial.provider_user_id).split("@")[0] + "_" + usuarioSocial.provider
    ...
    console.error("[v0] Erro no callback OAuth Facebook:", error)
